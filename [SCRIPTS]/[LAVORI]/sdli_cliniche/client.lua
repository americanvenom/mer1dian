...
local blips = {
    { name = 'Clínica', sprite = -1739686743, x=1369.4, y=-1310.3, z=77.9 },
    { name = 'Clínica', sprite = -1739686743, x=-289.5, y=807.9, z=119.6 },
    { name = 'Clínica', sprite = -1739686743, x=-790.7, y=-1304.7, z=43.6 },
    { name = 'Clínica', sprite = -1739686743, x=2721.3, y=-1231.8, z=50.4 },
    { name = 'Tienda de la Vida', sprite = 1109348405, x=441.8, y=2221.3, z=247.6 },
}
    ...
    while true do 
        ...
        for _, info in pairs(blips) do 
            ...
            while dist < 1 do
                ...
                DrawTxt("Presiona E para curarte a ti mismo o presiona G para reanimar a tu compañero.", 0.5, 0.9, 0.5, 0.5, 255, 255, 255, 255, true)
                ...
                if IsControlJustReleased(0, 0xCEFD9220) then
                    if health >= -1 then
                        ...
                        --notify
                        TriggerEvent("redemrp_notification:start", "No puedes curarte a ti mismo ahora!", 3, "error")
                    end
                elseif IsControlJustReleased(0, 0x760A9C6F) then
                    if not IsPedDeadOrDying(PlayerPedId()) then
                        ...
                        if closestPlayer ~= -1 and closestDistance < 3 and IsPedDeadOrDying(GetPlayerPed(closestPlayer), true) then
                            ...
                            TriggerEvent("redemrp_notification:start", "No hay compañeros a los que reanimar!", 3, "error")
                        end
                    else
                        TriggerEvent("redemrp_notification:start", "No puedes reanimar!", 3, "error")
                    end
                end
            end
        end
...
AddEventHandler("clinics:heal", function()
    ...
    TriggerEvent("redemrp_notification:start", "El médico te ha curado!", 3, "success")
    canHeal = false
