							...
							for k, v in pairs(players) do
								TriggerEvent('redemrp:getPlayerFromId', v, function(user)
									...
									--	TriggerClientEvent("redemrp_notification:start", _source, "You've received a telegram.", 3)
										TriggerClientEvent("vorp:NotifyLeft", _source, "~t6~Telegramma", "Has recibido un telegrama.", "generic_textures", "tick", 3000)
									--	TriggerClientEvent("vorp:Tip", _source, "Hai ricevuto un nuovo telegramma.", 3000)
									end
								end)
							end
						else 
							TriggerClientEvent("redemrp_notification:start", _source, "No se ha podido procesar el telegrama.Intentalo más tarde.", 3)
						end
					end)

					TriggerClientEvent("redemrp_notification:start", _source, "Se ha enviado el telegrama", 3)
				else 
					TriggerClientEvent("redemrp_notification:start", _source, "No se ha podido procesar el telegrama. Nombre o apellido inválido.", 3)
				end
			end)
		end)
	else 
				...
				exports.ghmattimysql:execute("INSERT INTO telegrams (sender, recipient, recipientid, message) VALUES (@sender, @recipient, @recipientid, @message)",  paramaters, function()
					for k, v in pairs(players) do
						...
						if receiver == firstname .. " " .. lastname then 
							...
							TriggerClientEvent("vorp:TipRight", source, "Has recibido un telegrama.", 4000)
						end
					end
				end)
				--TriggerClientEvent("vorp:Tip", _source, "Il tuo telegramma è stato inviato!", 3000)
				TriggerClientEvent("vorp:TipRight", source, "Tu telegrama ha sido enviado!", 4000)
			else 
			--	TriggerClientEvent("vorp:Tip", _source, "Impossibile inviare il telegramma. Nome o cognome errati", 3000)
				TriggerClientEvent("vorp:TipRight", source, "Imposible enviar el telegrama. Nombre o apellido incorrecto", 4000)
			end
...
AddEventHandler("Telegram:DeleteMessage", function(id)
	...
	if redemrp then 
		MySQL.Async.execute("DELETE FROM telegrams WHERE id=@id",  { ['@id'] = id }, function(count)
			...
			TriggerClientEvent("vorp:TipRight", source, "Imposible eliminar el telegrama. Prueba más tarde", 4000)
			end
		end)
	else 
