...
AddEventHandler('poke_rpchat:sendProximityMessage', function(playerId, title, message, color)
	...
	if target == player or #(playerCoords - targetCoords) < 20 then
		TriggerEvent('chatMessage',"", {255, 255, 255}, "^*"..title .. " dice: ^r".. message)
	end
end)


RegisterNetEvent('poke_rpchat:sendProximityMessageMe')
AddEventHandler('poke_rpchat:sendProximityMessageMe', function(playerId, title, message, color)
	local player = PlayerId()
	local target = GetPlayerFromServerId(playerId)
	...
	if target == player or #(playerCoords - targetCoords) < 20 then
		TriggerEvent('chatMessage',"",  {255, 255, 0},"^6^*".. title .." ^6^*".. message)
	end
end)

RegisterNetEvent('poke_rpchat:sendProximityMessageDo')
AddEventHandler('poke_rpchat:sendProximityMessageDo', function(playerId, title, message, color)
	local player = PlayerId()
	local target = GetPlayerFromServerId(playerId)
	...
	if target == player or #(playerCoords - targetCoords) < 20 then
		TriggerEvent('chatMessage',"", {255, 255, 0}, "^2^*" .. title .. " ^2^*".. message)
	end
end)
