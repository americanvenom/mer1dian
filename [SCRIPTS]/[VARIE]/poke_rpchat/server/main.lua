...
--         TriggerClientEvent('poke_rpchat:sendProximityMessage', -1, source, name, message, {255, 255, 255})
--     end
--     CancelEvent()
-- end)
...
AddEventHandler('chatMessage', function(source, playerName, message)
    if string.sub(message, 1, string.len("/")) ~= "/" then
        ...
        TriggerClientEvent('poke_rpchat:sendProximityMessage', -1, source, playerName, message, {255, 255, 255})
    end
    CancelEvent()
end)
...
RegisterCommand('me', function(source, args, rawCommand)
    ...
    local Character = User.getUsedCharacter
    local playerName = Character.firstname..' '..Character.lastname
    TriggerClientEvent('poke_rpchat:sendProximityMessageMe',  -1, source, playerName, args, {170, 102, 204})
end, false)
...
RegisterCommand('do', function(source, args, rawCommand)
    ...
    local Character = User.getUsedCharacter
    local playerName = Character.firstname..' '..Character.lastname
    TriggerClientEvent('poke_rpchat:sendProximityMessageDo', -1, source, playerName, args,{131, 153, 74})
end, false)
