...
Citizen.CreateThread(function() 
	while true do 
		...
		if tonumber(roundtemp) <= -8 then
			...
			TriggerEvent("vorp:TipBottom", "Te estás congelando, ponte algo más de ropa o busca refugio!", 4000)
		...
		elseif tonumber(roundtemp) <= -6 then 
			...
			TriggerEvent("vorp:TipBottom", "Tienes mucho frío, ¡deberías cubrirte más!", 4000)
		...
		--.	TriggerEvent("redemrp_notification:start", "Stai sentendo molto freddo, dovresti coprirti di più!", 5, "error")
			TriggerEvent("vorp:TipBottom", "Tienes  frío, ¡deberías cubrirte!", 4000)
	--	elseif tonumber(roundtemp) <= 20 then 
			--TriggerEvent("vorp:TipBottom", "Hace calor y empiezas a tener sed, ¡no vendría mal un poco de agua!", 4000)
